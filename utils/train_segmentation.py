...
for epoch in range(opt.nepoch):
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        loss = F.nll_loss(pred, target) # softmax+nll_loss = cross_entry loss
...
for i,data in tqdm(enumerate(testdataloader, 0)):
    ...
    for shape_idx in range(target_np.shape[0]): # 遍历一个batch里面的每一个样本
        parts = range(num_classes)# 遍历分割的每一类
        part_ious = []
        for part in parts: #
